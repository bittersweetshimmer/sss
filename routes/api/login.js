...
export default (db, session_config) => async (request, response) => {
    ...
    try {
        ...
        if (users_in_db.length != 1) { throw { error: 'invalid username or password' }; }

        const user = users_in_db[0];

        const password_matches = await bcrypt.compare(password, user.password_hash).catch(_ => { throw { error: 'internal error' }; });
        if (!password_matches) { throw { error: 'invalid username or password' }; }
        ...
        console.log(`${username} log in error: ${JSON.stringify(error)}`);
